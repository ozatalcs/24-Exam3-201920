...
def problem5(seq_of_seq):
    ...
    for j in range(len(seq_of_seq)):
        if len(seq_of_seq[j]) > 0:
            largest_int = seq_of_seq[j][0]
            for k in range(len(seq_of_seq[j])):
                if seq_of_seq[j][k] > largest_int:
                    largest_int = seq_of_seq[j][k]
            returned_list = returned_list + [largest_int]
